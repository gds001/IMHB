...
fig,axes=plt.subplots(2,1,sharex='col',sharey='row',figsize=(3,5))
def RMSD(e,f):
...
fitweI=np.array([0.4276611,0.02367607,1])
print("Ee vs. dw:", fitwe.coef_[0],fitwe.intercept_)
print(fitweI)
axes[0].set_xlim(0,1000)
axes[0].scatter(dw+fitweI[2]*dwm,de,c=colors,marker='.')
axes[0].plot(ws,ws*fitwe.coef_[0]+fitwe.intercept_,c='k',linestyle='--')
axes[0].plot(ws,Iogansen(ws,0,*fitweI),c='k')
axes[0].text(1000*(1-scale),14.5*scale,"RMSD:\n{:.2f} kcal/mol".format(RMSD(de,Iogansen(dv,dvm,*fitweI))),
               ha='right',va='bottom',fontsize=10)

fitwo=lm.LinearRegression().fit(np.array([dw+dwm]).T,deoh)
fitwoI=np.array([ 0.38706397,10.58126964,1.        ])
print("Eo vs. dw:", fitwo.coef_[0],fitwo.intercept_)
print(fitwoI)
axes[1].set_xlim(0,1000)
axes[1].scatter(dw+fitwoI[2]*dwm,deoh,c=colors,marker='.')
axes[1].plot(ws,ws*fitwo.coef_[0]+fitwo.intercept_,c='k',linestyle='--')
axes[1].plot(ws,Iogansen(ws,0,*fitwoI),c='k')
axes[1].text(1000*(1-scale),14.5*scale,"RMSD:\n{:.2f} kcal/mol".format(RMSD(deoh,Iogansen(dv,dvm,*fitwoI))),
               ha='right',va='bottom',fontsize=10)
...
axes[0].set_ylabel("$\\Delta E_e$ (kcal/mol)")
axes[1].set_ylabel("$\\Delta E_0$ (kcal/mol)")
...
plt.savefig("Figures/2DFitIFix_Anharm.png",dpi=600)
plt.show()
