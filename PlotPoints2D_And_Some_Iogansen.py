...
axes[1,0].set_xlabel("$\Delta r_\\mathrm{HB}+$"+"${:.2f}$".format(fitrI[4])+"$\\Delta r_\\mathrm{M}$\n(pm)")

fitro=lm.LinearRegression().fit(np.array([dr,drm]).T,deoh)

axes[1,1].set_xlabel("$\Delta \\omega_\\mathrm{HB}+$"+"${:.2f}$".format(fitwI[4])+"$\\Delta \\omega_\\mathrm{M}$\n(cm$^{-1}$)")
fitwo=lm.LinearRegression().fit(np.array([dw,dwm]).T,deoh)
...
plt.savefig("Figures/2DFitI.png",dpi=600)
plt.show()